...
| **Transcript polishing**         | The moment recording stops, an LLM pass turns the raw Web Speech word-stream into readable prose — adds punctuation and capitalization, splits run-ons into sentences, and collapses the adjacent-word duplications the browser STT emits — while preserving the candidate's voice, facts, and first-person framing. The polished text lands in an editable box so it can be corrected before evaluation. |
...
**Polish on stop, not on submit.** Transcript cleaning fires the moment recording ends and writes into an editable field, so the candidate reviews and fixes the text *before* it is scored — and the polished/edited version is what gets evaluated and persisted, never silently re-cleaned over the user's edits. The cleaning prompt is constrained to mechanical fixes (punctuation, capitalization, de-duplication) and is explicitly forbidden from paraphrasing or inventing content, so it can't quietly improve the answer the interview is meant to assess.
