...
def clean_transcript(raw: str) -> str:
    prompt = (
        "You are a transcript editor. The input is raw speech-to-text output: no "
        "punctuation, no capitalization, with fillers and stutters. Turn it into clean, "
        "readable written prose WITHOUT changing what the speaker actually said.\n\n"
        "Rules (follow all of them):\n"
        "1. Add punctuation: commas, periods, question marks, and apostrophes. Split the "
        "run-on word stream into proper sentences.\n"
        "2. Capitalize the first word of each sentence and proper nouns (names, companies, "
        "technologies, places).\n"
        "3. Remove filler words: um, uh, like, you know, so, yeah, right, basically, literally.\n"
        "4. Remove false starts (e.g. 'I, I worked' → 'I worked'; 'we, we built' → 'we built').\n"
        "5. Remove immediate word-for-word repetitions (e.g. 'the the project' → 'the project', "
        "'I built built it' → 'I built it'). Speech-to-text often duplicates adjacent words — "
        "collapse every such duplication FIRST, before punctuating. A repeated word is a stutter, "
        "never a standalone sentence: 'why why do you want it' → 'Why do you want it?', NOT "
        "'Why? Why do you want it?'.\n"
        "6. Fix only the minimal grammar needed for the text to read as correct written "
        "English (verb agreement, obvious wrong articles). Do NOT rephrase, reorder, or "
        "upgrade word choice beyond that.\n"
        "7. Keep first-person voice exactly as-is ('I', 'we', 'my', 'our') and keep every fact, "
        "name, number, and claim from the original.\n"
        "8. Do NOT add any new content, context, or explanation that was not in the original.\n"
